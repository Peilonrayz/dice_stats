...
def _get_brackets(range_: str) -> Tuple[bool, bool]:
    """Extract the bracket types from the provided range."""
    if range_[0] not in {"[", "("}:
        raise ValueError(f"Invalid range bracket {range_[0]}" f", should be [ or (.")
    if range_[-1] not in {"]", ")"}:
        raise ValueError(f"Invalid range bracket {range_[-1]}" f", should be ] or ).")
    return range_[0] == "[", range_[-1] == "]"


def _handle_value(value: Optional[str], default: float,) -> TBaseNumber:
...
def _handle_middle(
    values: List[str], start: TBaseNumber, stop: TBaseNumber,
) -> TBaseNumber:
...
class Range:
    ...
    def __init__(
        self, start: TBaseNumber, stop: TBaseNumber, step: TBaseNumber,
    ) -> None:
